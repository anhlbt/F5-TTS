...
DEFAULT_TTS_MODEL_CFG = [
    "/workspace/F5-TTS/ckpts/ljspeech_dataset/model_last.pt",
...
try:
    ...
    try:
        ...
        if nltk_data_path:
            ...
            nltk.download(
                "punkt", quiet=False
            )  # Download explicitly if missing, show output
        # Verify download
        nltk.data.find("tokenizers/punkt")
        print("'punkt' resource downloaded successfully.")
    except Exception as download_e:
        print(
            f"Failed to download NLTK 'punkt' resource: {download_e}. Text splitting may fall back to paragraphs."
        )
...
from mutagen.id3 import (
    ...
    TIT2,
    TPE1,
)  # <<< Added TIT2 (Title) and TPE1 (Artist/Author)
...
F5TTS_ema_model = load_custom(
    ...
    model_cfg=DEFAULT_TTS_MODEL_CFG[2],
)
...
def extract_metadata_and_cover(ebook_path):
    ...
    try:
        ...
        if (
            ...
            print(
                f"Failed to extract cover. RC: {result.returncode}, Stderr: {result.stderr}, Stdout: {result.stdout}"
            )
        ...
        print(
            f"Error: 'ebook-meta' command not found. Is Calibre installed and in PATH?"
        )
        if cover_path and os.path.exists(cover_path):
            try:
                os.remove(cover_path)
...
def embed_metadata_into_mp3(mp3_path, cover_image_path, title, author):
    ...
    if cover_image_path and os.path.exists(cover_image_path):
        ...
        print(
            "Cover image path invalid/missing or file doesn't exist. Skipping cover embedding."
        )
...
def extract_text_title_author_from_epub(epub_path):
    ...
    try:
        ...
        if metadata_creator:
            author = ", ".join([creator[0] for creator in metadata_creator])
        else:
            author_meta = book.get_metadata("OPF", "creator")
            if author_meta:
                author = author_meta[0][0]
            else:
                print("Author metadata ('DC:creator') not found. Keeping default.")
    ...
    if spine_ids:
        ...
        print(
            "Warning: EPUB spine information missing or empty. Processing items in default order."
        )
    ...
    for item in ordered_items:
        try:
            ...
            print(
                f"Warning: Error parsing item {item.get_id()} (Href: {item.get_name()}): {e}"
            )

    if not text_content:
        raise ValueError(f"No text could be extracted from EPUB: {epub_path}")
    full_text = "\n\n".join(text_content)
...
def convert_to_epub(input_path, output_dir):
    ...
    try:
        ...
        if result.returncode != 0:
            print(
                f"Warning: ebook-convert finished with return code {result.returncode}, but output file exists. Stderr: {result.stderr}, Stdout: {result.stdout}"
            )
        print(f"Successfully converted to '{output_path}'.")
        return output_path
    except FileNotFoundError:
        raise RuntimeError(
            f"Error: 'ebook-convert' command not found. Is Calibre installed and in PATH?"
        )
    except Exception as e:
...
def detect_file_type(file_path):
    ...
    try:
        ...
        if mime_type is None:
            print(
                f"Warning: python-magic returned None for {file_path}. Attempting fallback detection."
            )
...
def split_text_into_chunks(text, max_length=MAX_CHUNK_LENGTH):
    ...
    try:
        ...
        print(
            f"NLTK sentence tokenization failed: {e}. Falling back to paragraph/line split."
        )
        ...
        if not sentences and len(text) > max_length:
            print(
                f"Text has no clear paragraphs/lines. Using fixed-length split of {max_length} chars as final fallback."
            )
            sentences = [
            ...
            sentences = [text]
            print("Text is short and no splits found. Treating as single chunk.")
    ...
    for sentence in sentences:
        ...
        if len(sentence) > max_length:
            print(
                f"Warning: Sentence exceeds max_length ({len(sentence)} > {max_length}). Splitting mid-sentence."
            )
            if current_chunk:
    ...
    if chunks:
        ...
        print(
            f"Chunk lengths: Min={min(lengths)}, Max={max(lengths)}, Avg={sum(lengths)/len(lengths):.2f}"
        )
    return chunks
...
@gpu_decorator
def infer_chunk(
    ref_audio_orig, ref_text, text_chunk, cross_fade_duration, speed, show_info
):
    ...
    try:
        ...
        if not ref_text and not processed_ref_text:
            raise RuntimeError(
                "Automatic transcription of reference audio failed or produced empty text. Cannot proceed with TTS."
            )
        elif not ref_text and processed_ref_text:
            print("Automatic transcription successful.")
    except Exception as e:
        import traceback

        print(
            f"Error during reference audio/text preprocessing: {e}\n{traceback.format_exc()}"
        )
        raise RuntimeError(f"Preprocessing failed: {e}")
    if not text_chunk or text_chunk.isspace():
        print("Skipping empty or whitespace-only text chunk.")
        return None
    try:
        with torch.no_grad():
        ...
        if final_wave is None or final_sample_rate is None:
            print("Warning: TTS inference returned None for wave or sample rate.")
            return None
        if isinstance(final_wave, torch.Tensor) and final_wave.numel() == 0:
            print("Warning: TTS inference returned an empty tensor.")
            return None
        if isinstance(final_wave, np.ndarray) and final_wave.size == 0:
            print("Warning: TTS inference returned an empty numpy array.")
            return None
        print(
            f"Generated chunk: {len(final_wave) if hasattr(final_wave, '__len__') else 'N/A'} samples @ {final_sample_rate} Hz."
        )
        ...
        print(
            f"Error during TTS inference (infer_process): {e}\n{traceback.format_exc()}"
        )
        return None
...
def process_ebook_to_audio(
    ...
    try:
        if not ebook_path or not os.path.exists(ebook_path):
            yield None, f"Error: Input file not found or path is invalid: {ebook_path}"
            return
        ...
        if file_type != "application/epub+zip":
            ...
            try:
                ...
                print(f"Using converted EPUB: {epub_path_to_process}")
            except Exception as e:
                yield None, f"Error: Conversion to EPUB failed: {e}"
                return
        else:
            epub_path_to_process = ebook_path
            print("Input is already EPUB. No conversion needed.")
        ...
        try:
            ...
            print(
                f"Cover path: {extracted_cover_path}"
                if extracted_cover_path
                else "No cover found or extracted."
            )
        except ValueError as ve:
            yield None, f"Error: Failed to extract text content: {ve}"
            return
        except Exception as e:
            import traceback

            yield None, f"Error: Failed to extract content/metadata: {e}\n{traceback.format_exc()}"
            return
        ref_text = ref_text_input
        print(
            "Using provided reference text."
            if ref_text
            else "Reference text is empty. Automatic transcription will be attempted."
        )
        sanitized_title = sanitize_filename(ebook_title)
        sanitized_author = sanitize_filename(ebook_author)
        if not sanitized_title:
            sanitized_title = f"audiobook_{os.path.splitext(os.path.basename(original_input_path))[0]}"
        base_filename = (
            f"{sanitized_title}_by_{sanitized_author}"
            if sanitized_author and sanitized_author.lower() != "unknown_author"
        ...
        print(f"Planned output path: {final_mp3_path}")
        progress(0.2, desc="Splitting text...")
        text_chunks = split_text_into_chunks(gen_text, max_length=max_chunk_length)
        total_chunks = len(text_chunks)
        if total_chunks == 0:
            yield None, "Error: Text splitting resulted in zero chunks."
            return
        temp_dir = tempfile.mkdtemp(prefix="audio_chunks_")
        print(f"Temporary directory for audio chunks: {temp_dir}")
        successful_chunks = 0
        first_chunk_processed = False
        dummy_show_info = lambda *args, **kwargs: None
        for i, chunk in enumerate(text_chunks):
            chunk_start_progress = 0.25 + (i / total_chunks) * 0.5
            progress(
                chunk_start_progress,
                desc=f"Generating audio: Chunk {i+1}/{total_chunks}",
            )
            print(f"\nProcessing Chunk {i+1}/{total_chunks} (Length: {len(chunk)})")
            clean_chunk = chunk.strip()
            clean_chunk = re.sub(r"\s+", " ", clean_chunk)
            if not clean_chunk:
                print("Skipping empty or whitespace-only chunk.")
                continue
            audio_out_chunk_data = None
            try:
                audio_out_chunk_data = infer_chunk(
                    ref_audio_input,
                    ref_text,
                    clean_chunk,
                    cross_fade_duration,
            ...
            except RuntimeError as infer_err:
                print(f"Chunk {i+1} Error (from infer_chunk): {infer_err}")
            except Exception as generic_infer_err:
                print(
                    f"Chunk {i+1} Unexpected Error during inference call: {generic_infer_err}"
                )
                    ...
                    try:
                        ...
                        if wave.dtype != np.float32 and wave.dtype != np.int16:
                            wave = (
                                ...
                                and np.max(np.abs(wave)) <= 1.0
                                else (
                                    wave.astype(np.int16)
                                    if np.issubdtype(wave.dtype, np.integer)
                                    else wave.astype(np.float32)
                                )
                            )
                        if not first_chunk_processed:
                            sample_rate = chunk_sample_rate
                            print(
                                f"Audio sample rate set to {sample_rate} Hz (from first chunk)."
                            )
                            first_chunk_processed = True
                        elif chunk_sample_rate != sample_rate:
                            print(
                                f"Warning: Sample rate mismatch! Chunk {i+1} has SR={chunk_sample_rate}, expected {sample_rate}."
                            )
                        ...
                        print(f"Error saving chunk {i+1} WAV: {e}")
                else:
                    print(
                        f"Warning: Inference for chunk {i+1} produced invalid or empty audio data."
                    )
            else:
                print(f"Warning: Inference failed or was skipped for chunk {i+1}.")
            del audio_out_chunk_data, wave
            gc.collect()
            if device == torch.device("cuda"):
                torch.cuda.empty_cache()
        if not temp_chunk_files:
            yield None, "Error: No audio chunks were successfully generated."
            return
        print(
            f"\nFinished generating audio chunks. Successful: {successful_chunks}/{total_chunks}."
        )
        progress(0.8, desc="Stitching audio...")
        print(f"Concatenating audio chunks using ffmpeg (Bitrate: {mp3_bitrate})...")
        ...
        try:
            ...
            yield None, f"Error creating ffmpeg file list: {e}"
            return
        ffmpeg_cmd = [
            "ffmpeg",
            "-y",
        ...
        try:
            ...
            if proc.returncode != 0:
                error_message = f"ffmpeg concatenation error (RC={proc.returncode}):\n{proc.stderr or proc.stdout}"
                print(error_message)
            ...
            yield None, "Error: 'ffmpeg' command not found. Is ffmpeg installed and in PATH?"
            return
        except Exception as e:
            yield None, f"Error running ffmpeg: {e}"
            return
        ...
        if os.path.exists(final_mp3_path):
            ...
            print(
                f"Warning: Final MP3 file '{final_mp3_path}' not found after ffmpeg process. Skipping metadata embedding."
            )
        progress(1, desc=f"Completed: {os.path.basename(final_mp3_path)}")
        print(f"Successfully created audiobook: {final_mp3_path}")
        yield final_mp3_path, None
    except Exception as e:
        import traceback

        error_msg = f"Unexpected error processing {os.path.basename(ebook_path)}: {e}\n{traceback.format_exc()}"
        ...
        if temp_dir and os.path.exists(temp_dir):
            try:
                ...
                print(f"Warn: Could not remove temp dir {temp_dir}: {e}")
        if converted_epub_path and os.path.exists(converted_epub_path):
            try:
                os.remove(converted_epub_path)
                print(f"Removed temp EPUB: {converted_epub_path}")
            except Exception as e:
                print(f"Warn: Could not remove temp EPUB {converted_epub_path}: {e}")
        if (
            ...
            try:
                ...
                print(f"Removed temp cover: {extracted_cover_path}")
            except Exception as e:
                print(f"Warn: Could not remove temp cover {extracted_cover_path}: {e}")
        gc.collect()
        if device == torch.device("cuda"):
            torch.cuda.empty_cache()
...
def batch_process_ebooks(
    ...
    if not ref_audio_input and not ref_text_input:
        ...
        yield None, show_converted_audiobooks()
        return
    elif not ref_audio_input and ref_text_input:
        print("Proceeding with reference text only.")
        gr.Warning("Reference Audio missing. TTS may be suboptimal.")
    processed_paths = []
    last_successful_output_path = None
    errors = []
    ebook_paths = []
    if isinstance(gen_file_inputs, list):
        ebook_paths = [f.name for f in gen_file_inputs if hasattr(f, "name") and f.name]
    elif hasattr(gen_file_inputs, "name") and gen_file_inputs.name:
        ebook_paths = [gen_file_inputs.name]
    if not ebook_paths:
        gr.Warning("No valid eBook file paths found.")
        yield None, show_converted_audiobooks()
        return
    num_ebooks = len(ebook_paths)
    print(f"Starting batch processing for {num_ebooks} eBook(s)...")
    print(
        f"Settings: Speed={speed}, CrossFade={cross_fade_duration}, ChunkLen={max_chunk_length}, Bitrate={mp3_bitrate}"
    )
    for idx, ebook_path in enumerate(ebook_paths):
        ...
        try:
            ...
            if error_msg:
                error_details = (
                    f"'{os.path.basename(ebook_path)}': Failed - {error_msg}"
                )
                print(f"ERROR: {error_details}")
                errors.append(error_details)
                yield last_successful_output_path, show_converted_audiobooks()
            elif final_path and os.path.exists(final_path):
                success_message = f"'{os.path.basename(ebook_path)}': OK -> '{os.path.basename(final_path)}'"
                print(f"SUCCESS: {success_message}")
                processed_paths.append(final_path)
                last_successful_output_path = final_path
                yield last_successful_output_path, show_converted_audiobooks()
            else:
                unknown_error = f"'{os.path.basename(ebook_path)}': Finished, but no output file generated."
                print(f"WARNING: {unknown_error}")
                errors.append(unknown_error)
                yield last_successful_output_path, show_converted_audiobooks()
        except StopIteration:
            stop_iter_warn = f"'{os.path.basename(ebook_path)}': Processing generator finished unexpectedly."
            print(f"WARNING: {stop_iter_warn}")
            errors.append(stop_iter_warn)
            yield last_successful_output_path, show_converted_audiobooks()
        except Exception as e:
            import traceback

            batch_loop_error = f"Batch loop error for '{os.path.basename(ebook_path)}': {e}\n{traceback.format_exc()}"
            print(f"ERROR: {batch_loop_error}")
            errors.append(
                f"'{os.path.basename(ebook_path)}': Critical batch error - {e}"
            )
            yield last_successful_output_path, show_converted_audiobooks()
    print("\n--- Batch Processing Complete ---")
    final_audiobook_list = show_converted_audiobooks()
    if errors:
        error_summary = "Batch processing finished with errors:\n" + "\n".join(errors)
        gr.Warning(error_summary)
        print("\nSummary of Errors:")
...
def clear_ref_text_on_audio_change(audio_filepath):
    if audio_filepath and os.path.exists(audio_filepath):
        print(
            f"Ref audio changed ({os.path.basename(audio_filepath)}). Clearing ref text."
        )
        return ""
    else:
        print("Ref audio cleared. Ref text cleared.")
        return ""
...
def create_gradio_app():
    ...
    try:
        ...
        print("Soft theme loaded.")
    except AttributeError:
        print("Warning: Soft theme not found, using Default.")
        theme = gr.themes.Default()
    default_audio_exists = os.path.exists(DEFAULT_REF_AUDIO_PATH)
    if not default_audio_exists:
        print(f"Warning: Default ref audio not found: {DEFAULT_REF_AUDIO_PATH}")
    available_bitrates = ["128k", "192k", "256k", "320k"]
    default_bitrate = "320k"
    default_chunk_length = MAX_CHUNK_LENGTH
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, min_width=350):
                ...
                max_chunk_length_input = gr.Slider(
                    ...
                    minimum=100,
                    maximum=1500,
                    value=default_chunk_length,
                    step=50,
                )
                mp3_bitrate_input = gr.Dropdown(
                    label="Output MP3 Bitrate (Quality)",
            ...
            with gr.Column(scale=2, min_width=400):
                ...
                player = gr.Audio(
                    label="Listen to the Latest Audiobook", interactive=False
                )
                ...
                generate_btn = gr.Button(
                    "Generate Audiobook(s)", variant="primary", scale=2
                )  # scale might need adjustment here
...
@click.option(
    "--host", "-H", default="0.0.0.0", help="Host address (0.0.0.0 for Docker)."
)
@click.option(
    "--share", "-s", default=False, is_flag=True, help="Share via Gradio link."
)
...
def main(port, host, share, debug):
    ...
    if not os.path.exists(DEFAULT_REF_AUDIO_PATH):
        print(f"WARN: Default ref audio missing: {DEFAULT_REF_AUDIO_PATH}")
    else:
        print(f"Default ref audio found: '{DEFAULT_REF_AUDIO_PATH}'")
    try:
        result = subprocess.run(
        ...
        print(f"ERROR: ffmpeg check failed: {e}. Audio stitching will likely fail.")
    try:
        calibre_path = "/root/calibre/ebook-convert"
        cmd = [
            calibre_path if os.path.exists(calibre_path) else "ebook-convert",
            "--version",
        ]
        result = subprocess.run(cmd, check=True, capture_output=True, text=True)
        print(f"ebook-convert found. ({result.stdout.strip()})")
    except Exception as e:
        print(
            f"ERROR: ebook-convert check failed: {e}. Conversion for non-EPUB formats will fail."
        )
    try:
        nltk.data.find("tokenizers/punkt")
        ...
        print(
            "WARN: NLTK 'punkt' data not found. Sentence splitting may be less accurate."
        )
    print("--- End Pre-run Checks ---")
    app = create_gradio_app()
    print(f"\nStarting Gradio app on http://{host}:{port}")
    if share:
        print("Gradio share link will be created.")
    if debug:
        print("Gradio debug mode enabled.")
    try:
...
if __name__ == "__main__":
    ...
    if not USING_SPACES:
        ...
        print("Launching Gradio app in Spaces mode...")
        app = create_gradio_app()
