...
def create_metadata_v1(name_project, ch_tokenizer, progress=gr.Progress()):
    ...
    for line in progress.tqdm(data.split("\n"), total=count):
        sp_line = line.split("|")
        if len(sp_line) != 3:
            print("please check your metadata.csv")
            print(sp_line)
            continue
        try:
        ...
        if duration < 1 or duration > 31:
            if duration > 31:
                error_files.append([file_audio, "duration > 31 sec"])
            if duration < 1:
...
def process_audio_line(args):
    ...
    try:
        ...
        if duration < 1 or duration > 31:
            error_msg = "duration > 31 sec" if duration > 31 else "duration < 1 sec"
...
def calculate_train(
    ...
    if batch_size_type == "frame":
        mini_batch_duration = (
            batch_size_per_gpu * gpu_count * hop_length / sampling_rate
        )
...
def prune_checkpoint(
    checkpoint_path: str, new_checkpoint_path: str, save_ema: bool, safetensors: bool
) -> str:
    try:
...
```"""
            ...
            with gr.Row():
                exp_name = gr.Radio(
                    label="Model", choices=["F5TTS_v1_Base", "F5TTS_Base", "E2TTS_Base"]
                )
            ...
            with gr.Row():
                ...
                grad_accumulation_steps = gr.Number(
                    ...
                    info="Effective batch size is multiplied by this value",
                )
                max_samples = gr.Number(
                    label="Max Samples",
                    info="Maximum number of samples per single GPU batch",
                )
            ...
            with gr.Row():
                ...
                mixed_precision = gr.Radio(
                    label="Mixed Precision", choices=["none", "fp16", "bf16"]
                )
                cd_logger = gr.Radio(
                    label="Logger", choices=["none", "wandb", "tensorboard"]
                )
                with gr.Column():
...
```"""
            ...
            with gr.Row():
                ch_use_ema = gr.Checkbox(
                    label="Use EMA",
                    value=True,
                    info="Turn off at early stage might offer better results",
                )
